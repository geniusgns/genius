...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: genius.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: geniusd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizegeniusamount=<n> " + _("Keep N Genius anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Genius is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Genius version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeGeniusAmount = GetArg("-anonymizegeniusamount", 0);
    if(nAnonymizeGeniusAmount > 999999) nAnonymizeGeniusAmount = 999999;
    if(nAnonymizeGeniusAmount < 2) nAnonymizeGeniusAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeGeniusAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize Genius Amount %d\n", nAnonymizeGeniusAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1GENIUS+1000 == (.1GENIUS+100)*10
       10GENIUS+10000 == (1GENIUS+1000)*10
    */
