...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeGeniusAmount"))
        settings.setValue("nAnonymizeGeniusAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeGeniusAmount = settings.value("nAnonymizeGeniusAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeGeniusAmount"))
        SoftSetArg("-anonymizegeniusamount", settings.value("nAnonymizeGeniusAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeGeniusAmount:
            nAnonymizeGeniusAmount = value.toInt();
            settings.setValue("nAnonymizeGeniusAmount", nAnonymizeGeniusAmount);
            emit anonymizeGeniusAmountChanged(nAnonymizeGeniusAmount);
            break;
        default:
            break;
        }
